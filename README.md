...
4. **DB buses tab** — Discovery Bay map (C4/C9/6, DB01R/DB02R…). Live **stop ETAs** from `eta.dbtsl.com`. **No vehicle GPS** — bus icons are ETA-inferred on OSRM roads with heading. **C4 · C9 board** below the map: one vertical column per active bus with fixed landmarks (Coastline/Crestmont → Main Plaza → North Plaza), clock ETA + mins, and a blue ETA-inferred progress dot. Optional **your GPS** suggests nearest stop, walk time, next ETA, and likely direction.
